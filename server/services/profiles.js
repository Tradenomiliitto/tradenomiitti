module.exports = function initialize(params) {
  ...
  function listProfiles(loggedIn, limit, offset, filters = {}, order) {
    ...
    if (filters.institute !== undefined) {
      query = query.whereExists(function whereExists() {
        this.select('user_id')
          ...
          .whereRaw(
            "users.id = user_educations.user_id and data->>'institute' = ?",
            [filters.institute]
          );
      });
    }
    ...
    if (filters.specialization !== undefined) {
      query = query.whereExists(function whereExists() {
        this.select('user_id')
          .from('user_educations')
          .whereRaw(
            "users.id = user_educations.user_id and data->>'specialization' = ?",
    ...
    if (order === undefined || order === 'recent') {
      query = query
        ...
        .groupBy('users.id')
        .orderByRaw(
          'greatest(max(ads.created_at), max(answers.created_at), users.modified_at) desc nulls last'
        );
    }
  ...
  function profileEducations(user_id) {
    return knex('user_educations')
      .where({ user_id })
      .then(rows => rows.map(row => row.data));
  }

  function profileSpecialSkills(user_id) {
    return knex('user_special_skills')
      .where({ user_id })
      .then(rows => rows.map(row => row.heading));
  }
  ...
  function addContact(loggedInUser, toUserId, introductionText) {
    if (typeof introductionText !== 'string' || introductionText.length < 10) {
      return Promise.reject({
        status: 400,
        msg: 'Introduction text is mandatory',
      });
    }
    ...
    if (loggedInUser.id === toUserId) {
      return Promise.reject({
        status: 400,
        msg: 'User cannot add contact to himself',
      });
    }

    const businessCard = util.formatBusinessCard(
      loggedInUser.data.business_card
    );
    if (!businessCard) {
    ...
    return knex('contacts')
      .where({ from_user: loggedInUser.id, to_user: toUserId })
      .then(resp => {
        if (resp.length === 0) {
          return knex('contacts')
            .insert(
        ...
        return Promise.reject(
          'User has already given their business card to this user'
        );
      });
  }
