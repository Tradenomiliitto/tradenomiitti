...
export default function initAnimation(port) {
  port.subscribe(makeAnimation);
}

export const allAnimationIdsToReinit = [ 'home-intro-canvas', 'login-needed-canvas' ];

export function makeAnimation([ id, isSplashScreen ]) {
  // wait until element is rendered
  let counter = 0;
  const extraHeightFromMemberbar = $('#tral-memberbar').height() || 0;
  ...
  function init(canvas) {
    ...
    const offset = isSplashScreen ? 0 : navbarHeight + extraHeightFromMemberbar;
